...
void Session::initData()
{
    ...
    for (unsigned int i = 0; i < numEvents; ++i)
    {
        ...
        for (const auto& collectionName : frame.getAvailableCollections())
        {
            ...
            if (const auto* pTrackerHits = dynamic_cast<const edm4hep::SimCalorimeterHitCollection*>(pCollection))
            {
                for (const auto& hit : *pTrackerHits)
                    event.markers.push_back(std::make_shared<scene::objects::Marker>(collectionName));
                    // TODO: populate marker position from hit.getPosition()
            }
            else if (const auto* pCaloHits = dynamic_cast<const edm4hep::SimCalorimeterHitCollection*>(pCollection))
            {
